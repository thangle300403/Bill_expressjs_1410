...
export const sql_executor = async (state) => {
    ...
    // ✅ Nếu truy vấn có điều kiện email nhưng không có email người dùng
    const usesEmail = lowerSQL.includes("c.email") || lowerSQL.includes("customer.email");

    if (usesEmail && !state.email) {
        console.warn("❌ Email required but not provided");
        return {
            messages: [
                {
                    role: "ai",
                    content:
                        "⚠️ Bạn cần đăng nhập để truy xuất thông tin đơn hàng của mình. Vui lòng đăng nhập trước khi tiếp tục.",
                },
            ],
            current_step: "__end__",
            error: "Email required for this query",
        };
    }
...
`],
        ...
        const refined = await refineChain.invoke({ raw, schema, formattedHistory, userQuestion, context });
