...
`],
        ...
        log(`Bill gửi dữ liệu cho trợ lí quản lí cơ sở dữ liệu 1.`, "sql-generator");
        ...
        if (!sql) {
            console.warn("⚠️ SQL is null or not found in response.");
            return {
                current_step: "__end__",
                error: "No valid SQL generated",
                answered_intents: [...(state.answered_intents || []), "sql"], // ✅ Đánh dấu đã xử lý intent sql
            };
        }
...
export const result_refiner = async ({ messages, dbRows, session_id, history, intent, email, answered_intents = [], }) => {
    console.log("🚧 Executing: result_refiner");
    const logKey = email || session_id;
    const log = (msg, step = null) => pushLog(logKey, { msg, step });
    try {
        const allProducts = await getProductCache();
        const schema = await loadSchema();
        ...
        log(`Trợ lí lọc ra dữ liệu phù hợp.`, "sql-check-refiner");
...
`],
        ...
        log(`Trợ lí cố gắng nhớ lại lịch sử trò chuyện`, "sql-history-refiner");
        const formattedHistory = (history || [])
            .map(m => `${m.role === "user" ? "KH" : "AI"}: ${m.content}`)
            .join("\n");
        ...
        log(`Bill đang gởi dữ liệu cho trợ lí quản lí cơ sở dữ liệu 2`, "sql-refiner");

        const refined = await refineChain.invoke({
            raw,
            schema,
            formattedHistory,
            userQuestion
        ...
        log(`Đang xem kĩ lại câu trả lời`, "sql-final-refiner-matched");
        const matched = [];
        const seen = new Set();
