...
6.When user question is about ask to add product to cart, If add_to_cart is not in usedTools, then the add to cart proccess is not done, add_to_cart tool must be called by then.
IF there is a product in matchedProdInUserQues, then the product must be used in add_to_cart process.
...
Nhìn vào Conversation history, xem các tools đã sử dụng đáp ứng đủ câu hỏi của KH chưa, nếu chưa thì dùng tiếp
...
== Data Provided ==
- 🧠 Các ví dụ tham khảo gần giống từ bộ huấn luyện: {consultDecision}
...
export async function consultAgent({ messages, email, history, session_id, intent, answered_intents = [], original_user_msg, used_tool, cartProduct, matchedProdInUserQues = [], topMatchedProduct }) {
    try {
        ...
        console.log("🚧 userQuestion in consult:", userQuestion);

        const consultDecision = await searchSimilar(userQuestion, 3, 0.5, "consult_decision");

        console.log("🚧 consultDecision:", consultDecision);
        ...
        // ✅ Nếu đã có sản phẩm match (từ match_product hoặc add_to_cart)
        if (topMatchedProduct) {
            console.log("🛒 Using topMatchedProduct for card display");

            matched.push({
                id: topMatchedProduct.product_id,
                name: topMatchedProduct.name,
                price: topMatchedProduct.price,
                featured_image: topMatchedProduct.featured_image,
            });
        }

        // 🧠 Nếu sau này muốn hỗ trợ nhiều sản phẩm (ví dụ gợi ý 3 cái), 
        // bạn có thể mở rộng matched = productRecommendations thay thế.
