...
export function streamLogs(req, res) {
    ...
    const session_id =
        req.cookies?.chatbot_session || req.query.session_id || null;
    ...
    res.setHeader("Access-Control-Allow-Origin", process.env.FRONTEND_URL_NEXT);
...
export function streamAnonLogs(req, res) {
    const key = "anon"; // fixed bus for anon users
    const bus = getLogBus(key);
    ...
    const send = (msg) => {
        console.log("🚀 Sending anon SSE:", msg);
        res.write(`data: ${JSON.stringify(msg)}\n\n`);
    };
...
export function pushAnonLog(payload) {
    const key = "anon"; // static key, or random ID if you prefer
    const bus = getLogBus(key);
    const fullPayload = {
        ts: Date.now(),
        ...payload,
